...
function pytests {
    CD=$(pwd)
    cd $PROJ_BASE
    dorun "./manage.py test topicos" "Testes python"
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function copy2www {
    ...
    cd $PROJ_BASE/frontend
    frontprod
    mkdir -p ../topicos/static/
    cp -Rf dist/js dist/css ../topicos/static/
    cd $CCD
    return $exitcode
}
...
echo_green "Bem vinda, Mariane, customize essa mensagem com o q vc quiser:"
